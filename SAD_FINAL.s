vbsme:		
				li		$v0, 0					# Reset v0
				li		$v1, 0					# Reset v1
				lui 	$s7, 0x7fff				# SAD Comparison Value (Upper 4 Bytes)
				ori 	$s7, $s7, 0xffff		# SAD Comparison Value (Lower 4 Bytes)
				...
				addi	$s4, $s4, -4			#	Adjust back 4 Bytes to Account Start at Zero
				add     $s4, $s4, $a2       	# End of Window Address
				sll     $t7, $s1, 2	        	# Frame Cols Offset
				...
				addi	$t9, $t9, -4			#	Adjust back 4 Bytes to Account Start at Zero
				add     $s6,  $0, $0        	# Saves Last move of window(INIT: 0, DOWN: 1, RIGHT: 2, DL: 3, UR: 4)
SAD_Routine:	add		$t9, $a2, $s5			# Frame-Window Row End
				addi	$t9, $t9, -4			#	Adjust back 4 Bytes to Account Start at Zero
				addi	$t5,  $0, 0				# (Re)set SAD Window Total
				add	    $t3, $t6, $0        	# Current initial Frame address before window scan
				add     $t4, $a2, $0        	# (Re)set current Window address
WindowLoop:		lw		$t1, 0($t3)				# Frame Value
				...
				nor 	$t1, $t1, $0			# 
				addi	$t1, $t1, 1				# 
gtZero:			add 	$t5, $t5, $t1			# Window SAD Total
				...
				j 		WindowLoop
NextRow:		add		$t9, $t9, $s5			# Move to Next Row End
				addi 	$t4, $t4, 4				# Goto Next Window Element	
				add		$t3, $t3, $t7			# Frame Jump
				j		WindowLoop
returnUpdate:	addi    $t0,  $0, 0		
				sub     $t1, $t6, $a1        	# Compute Frame offset from starting address
				srl     $t1, $t1, 2         	# convert address offset to integer index
IndexLoop:		slt     $t8, $t1, $s1			# if(t0 < 0) t2 = 1 else if(t0 >= 0) t2 = 0
				bne		$t8,  $0, CheckSAD 		# conitnue loop until t0 <= 0, t2 = 0
				sub     $t1, $t1, $s1       	# division by subraction
				addi	$t0, $t0, 1
				j 		IndexLoop
CheckSAD:		slt 	$t8, $s7, $t5			# Check if current SAD is less than existing
				bne 	$t8,  $0, CheckExit 		#	Branch to Check if We Can Exit
        		addi    $s7, $t5, 0         	# Update SAD with new comparison value
				addi	$v0, $t0, 0				# Update Return Value
				addi	$v1, $t1, 0				# Update Return Value
CheckExit:		sub		$t2, $s1, $s3			# Calc Greatest Possible Column
				sub		$t3, $s0, $s2			# Calc Greatest Possible Row
				bne		$t0, $t3, NextWindow	# 
				bne     $t1, $t2, NextWindow
				j 		End
NextWindow:		bne		$s6,  $0, lastUR		# If Last Move was not init (0...0000), try Down
				beq		$s6,  $0, MoveRight		# Otherwise Move Right - FIX FOR POSSIBLE MOVE DOWN
lastUR:			andi	$t8, $s6, 4				# Test Value for Last Move Up & Right (0...0100)
...
lastDL:			andi	$t8, $s6, 8				# Test Value for Last Move Down & Left (0...1000)
				beq		$t8,  $0, lastDown		# If Last Move was Not Down & Left, try Down
...
lastDown:		andi	$t8, $s6, 1				# Test Value for Last Move Down (0...0001)
				...
				j		MoveDown
lastRight:		andi	$t8, $s6, 2				# Test Value for last Move Right (0...0010)
